...
// GET /api/notify/run — dipanggil oleh frontend setiap 30 detik
...
export async function GET() {
  ...
  try {
    ...
    for (const task of tasks) {
      ...
      const taskLabel = `${projName}${task.name}`
      ...
      if (msUntilReady > 0 && msUntilReady <= 130000 && !task.notifiedWarnAt) {
        ...
        try {
          ...
          console.error(`Telegram warn error for ${user.username} task ${task.id}:`, err)
          errorCount++
        }
      }
      ...
      if (msUntilReady <= 0 && !task.notifiedReadyAt) {
        try {
          ...
          console.error(`Telegram ready error for ${user.username} task ${task.id}:`, err)
          errorCount++
        }
      }
    }
    ...
    console.error('Notify run error:', e)
    return NextResponse.json({ error: 'Notify run failed', details: String(e) }, { status: 500 })
  }
}
